    ...
    <div class="container py-5">
        ...
        <div v-else-if="book" class="row g-5">
            ...
            <div class="col-md-7 col-lg-8">
                ...
                <div class="card-footer bg-white border-0 p-0 mb-4">
                    <button v-if="!authStore.isAdmin" class="btn btn-primary btn-lg px-5 fw-bold rounded-3 shadow-sm"
                    ...
                    <button v-else class="btn btn-secondary btn-lg px-5 fw-bold rounded-3" disabled>
        ...
        <div v-else class="text-center py-5">
            ...
            <router-link to="/books" class="btn btn-primary mt-3">Quay lại danh sách</router-link>
        </div>
    </div>
</template>
...
const fetchBookDetail = async () => {
    ...
    try {
        ...
        // SỬA LỖI TẠI ĐÂY: Trích xuất đúng object sách từ data.data
        if (res.data?.success && res.data?.data) {
            book.value = res.data.data;
        } else {
            book.value = res.data; // Trường hợp API cũ
        }
    } catch (error) {
        console.error("Lỗi fetchBookDetail:", error);
        // Nếu không tìm thấy sách, quay về trang danh sách
        router.push('/books');
    } finally {
...
const fetchSettings = async () => {
    try {
        ...
        // SỬA LỖI TẠI ĐÂY: Cấu trúc mới bọc trong data.data
        const settings = res.data?.data || res.data;
        if (settings) {
            maxBorrowBooks.value = settings.soSachMuonToiDa;
        }
    } catch (error) {
...
// Hàm định dạng tiền tệ an toàn (tránh NaN)
