<template>
  <div>
    ...
    <ul class="nav nav-pills mb-4 gap-2">
      <li class="nav-item">
        <button class="nav-link fw-semibold rounded-pill px-4" :class="{ active: currentTab === '' }" @click="changeTab('')">
          Tất cả
        </button>
      </li>
      <li class="nav-item">
        <button class="nav-link fw-semibold rounded-pill px-4 position-relative" :class="{ active: currentTab === 'CHO_DUYET' }" @click="changeTab('CHO_DUYET')">
          Chờ duyệt
          <span v-if="countPending > 0" class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
            {{ countPending }}
          </span>
        </button>
      </li>
      <li class="nav-item">
        <button class="nav-link fw-semibold rounded-pill px-4" :class="{ active: currentTab === 'DANG_MUON' }" @click="changeTab('DANG_MUON')">
          Đang mượn
        </button>
      </li>
      <li class="nav-item">
        <button class="nav-link fw-semibold rounded-pill px-4" :class="{ active: currentTab === 'DA_TRA' }" @click="changeTab('DA_TRA')">
          Đã trả
        </button>
      </li>
      <li class="nav-item">
        <button class="nav-link fw-semibold rounded-pill px-4" :class="{ active: currentTab === 'DA_HUY' }" @click="changeTab('DA_HUY')">
          Đã hủy
        </button>
      </li>
    </ul>
          ...
          <tbody>
            ...
            <tr v-else v-for="phieu in filteredBorrows" :key="phieu._id">
              ...
              <td>
                ...
                <div class="small text-muted"><i class="bi bi-telephone-fill me-1"></i>{{ phieu.maDocGia?.dienThoai }}</div>
              </td>

              <!-- Sách -->
              <td>
                <div class="d-flex align-items-center gap-2">
                  <img :src="getImageUrl(phieu.maSach?.hinhAnh)" class="rounded border" style="width: 30px; height: 45px; object-fit: cover;" @error="handleImageError">
                  <span class="fw-semibold text-dark text-truncate" style="max-width: 200px;">{{ phieu.maSach?.tenSach || 'Sách đã bị xóa' }}</span>
                </div>
              </td>
              ...
              <td class="text-center">
                ...
                <div v-if="phieu.tienPhat > 0" class="mt-1">
                  <span class="badge" :class="phieu.daThanhToanPhat ? 'bg-success bg-opacity-10 text-success' : 'bg-danger bg-opacity-10 text-danger'">
              ...
              <td class="text-end pe-4">
                ...
                <template v-if="phieu.trangThai === 'CHO_DUYET'">
                  <button class="btn btn-sm btn-success me-2 fw-semibold shadow-sm" @click="handleAction(phieu._id, 'approve', 'Duyệt mượn sách này?')">
                    <i class="bi bi-check2"></i> Duyệt
                  </button>
                  <button class="btn btn-sm btn-outline-danger fw-semibold" @click="handleAction(phieu._id, 'reject', 'Từ chối yêu cầu này?')">
                    Từ chối
                  </button>
                </template>
                ...
                <template v-if="phieu.trangThai === 'DA_TRA' && phieu.tienPhat > 0 && !phieu.daThanhToanPhat">
                  <button class="btn btn-sm btn-danger fw-semibold shadow-sm" @click="handleAction(phieu._id, 'pay-fine', `Xác nhận thu ${formatCurrency(phieu.tienPhat)} tiền phạt?`)">
