      ...
      <div class="col-12 col-sm-6 col-xl-3">
        <div class="card border-0 shadow-sm rounded-4 h-100">
          <div class="card-body p-4">
            ...
            <i
              class="bi bi-people-fill position-absolute top-50 end-0 translate-middle-y pe-4 fs-1 text-primary opacity-25"></i>
          </div>
        </div>
      </div>
      ...
      <div class="col-12 col-sm-6 col-xl-3">
        <div class="card border-0 shadow-sm rounded-4 h-100">
          <div class="card-body p-4">
            ...
            <i
              class="bi bi-book-fill position-absolute top-50 end-0 translate-middle-y pe-4 fs-1 text-success opacity-25"></i>
          </div>
        </div>
      </div>
      ...
      <div class="col-12 col-sm-6 col-xl-3">
        <div class="card border-0 shadow-sm rounded-4 h-100">
          <div class="card-body p-4">
            ...
            <i
              class="bi bi-bookmark-star-fill position-absolute top-50 end-0 translate-middle-y pe-4 fs-1 text-warning opacity-25"></i>
          </div>
        </div>
      </div>
      ...
      <div class="col-12 col-sm-6 col-xl-3">
        <div class="card border-0 shadow-sm rounded-4 h-100 bg-primary text-white"
          ...
          <div class="card-body p-4">
            ...
            <i
              class="bi bi-cash-stack position-absolute top-50 end-0 translate-middle-y pe-4 fs-1 text-white opacity-25"></i>
          </div>
        </div>
      </div>
    </div>
              ...
              <table class="table table-borderless align-middle mb-0">
                <tbody>
                  <tr v-for="(sach, index) in stats.topSach" :key="sach._id" class="border-bottom">
                    ...
                    <td>
                      <div class="fw-semibold text-dark text-truncate" style="max-width: 200px;">{{ sach.tenSach }}
                      </div>
                    </td>
                    <td class="text-end">
                  ...
                  <tr v-if="!stats.topSach || stats.topSach.length === 0">
...
const fetchStats = async () => {
  ...
  try {
    ...
    // SỬA LỖI TẠI ĐÂY: Trích xuất đúng dữ liệu từ cấu trúc mới
    const responseData = response.data?.success ? response.data.data : (response.data?.data || response.data);

    // Gán dữ liệu an toàn, đảm bảo mảng luôn tồn tại
    stats.value = {
      tongDocGia: responseData?.tongDocGia || 0,
      tongSach: responseData?.tongSach || 0,
      soLuotMuon: responseData?.soLuotMuon || 0,
      tongDoanhThu: responseData?.tongDoanhThu || 0,
      topSach: responseData?.topSach || [],
    ...
    if (stats.value.chartData && stats.value.chartData.length > 0) {
      ...
      // Reset biểu đồ nếu không có dữ liệu
      chartData.value = { labels: [], datasets: [] };
    }
  } catch (error) {
    console.error("Lỗi lấy thống kê:", error);
    // Trả về dữ liệu rỗng an toàn nếu API lỗi
    stats.value = { tongDocGia: 0, tongSach: 0, soLuotMuon: 0, tongDoanhThu: 0, topSach: [], chartData: [] };
  } finally {
...
const formatCurrency = (value) => {
  // Thêm fallback 0 để tránh lỗi định dạng NaN
